...
fr=open('E:\work\\clean_cut_test_file')
doc_terms_list=[]
doc_class_list=[]
class1_set=set()
class2_set=set()
...
for index,line in enumerate(fr):
    lv=line.strip().split('\001')
    cat=lv[0]
    doc_class_list.append(lv[0])
    doc_terms_list.append([i for i in lv[1:] if len(i)>1])
    for i in lv[1:] :
        if len(i.decode('utf-8'))>1:
            dd[i].add(cat)



ls=feature_selection(doc_terms_list,doc_class_list,'WLLR')
for i in  ls:
    print i ,dd[i]