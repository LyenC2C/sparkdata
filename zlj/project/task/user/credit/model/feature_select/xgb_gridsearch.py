...
def xgb_grid_search_cv(params, xgb_clf, X, Y):
    gsearch = GridSearchCV(estimator = xgb_clf, param_grid = params, scoring='roc_auc',cv=5)
    gsearch.fit(X,Y)
...
def xgb_fit(xgb_clf,X,Y, val_X,val_Y, useTrainCV=True, cv_folds=5, early_stopping_rounds=50):
    if useTrainCV:
    ...
    xgb_clf.fit(X, Y,eval_set=[(val_X,val_Y)], eval_metric='auc')

    #Predict training set:
#     dtrain_predictions = xgb_clf.predict(dval)
    dtrain_predprob = xgb_clf.predict_proba(val_X)[:,1]

    #Print model report:
    print "\nModel Report"
#     print "Accuracy : %.4g" % metrics.accuracy_score(Y, dtrain_predictions)
    print "AUC Score (val): %f" % metrics.roc_auc_score(val_Y, dtrain_predprob)
...
def create_xgb_classifier_from_params(params):
    xgb_clf = xgb.XGBClassifier(learning_rate = params['learning_rate'], n_estimators = params['n_estimators'], max_depth = params['max_depth'],
                                min_child_weight = params['min_child_weight'], gamma = params['gamma'], subsample = params['subsample'],
                                colsample_bytree = params['colsample_bytree'], reg_alpha = params['reg_alpha'],
                                reg_lambda = params['reg_lambda'],objective = 'binary:logistic', scale_pos_weight = 1, seed = 27)
    return xgb_clf
def create_optimum_xgb_classifier(X, Y,val_X,val_Y, n_passes = 4):
    optimum_params = {
    ...
    for iteration in range(0,n_passes):
        ...
        xgb_fit(xgb_clf, X, Y,val_X,val_Y)
    ...
    xgb_clf = create_xgb_classifier_from_params(optimum_params)
    xgb_fit(xgb_clf, X, Y,val_X,val_Y)
    xgb_fit(xgb_clf, X, Y,val_X,val_Y, useTrainCV=False)
