...
--step1 补全id
DROP  table t_zlj_dc_weibodata_final_3w_buquan_1025;
create table t_zlj_dc_weibodata_final_3w_buquan_1025 as
SELECT
   mid,
...
(
  SELECT *,NULL as next_user_name_id
    from t_zlj_dc_weibodata where length(next_user_name)<2 and ds=20161025
    UNION ALL
  SELECT
    t1.*,
    t2.idstr AS next_user_name_id
  FROM t_zlj_dc_weibodata t1 JOIN t_base_weibo_user t2 ON length(t1.next_user_name)>=2
                 and t1.next_user_name = t2.screen_name and t1.ds=20161019
) t
;
...
-- 产出数据量 2953683
DROP   TABLE t_zlj_dc_weibodata_3w_rs_105;
create table t_zlj_dc_weibodata_3w_rs_1025 as
SELECT mid,
  t3.origin_id_indeex ,
...
FROM
t_zlj_dc_weibodata_final_3w_buquan_1025  t1
...
create table t_zlj_dc_user_src_weibo_1025 as
select mid,t2.rn ,time,content from
t_zlj_dc_user_src_weibo t1
 join t_zlj_dc_weibodata_3w_username_id t2 on t1.orgin_id =t2.orgin_id   where ds='20161025' ;
-- 下面数据不用
-----------------------------------------------------------------------
