...
SELECT
 ...
 count(1) as num ,
round( log10(max(timespan)-min(min_timespan)+10),2) as time_cross ,
round( log10(std(case when timespan<0.5 then min_timespan else timespan end )+10),2) as time_std ,
...
select
  ...
  id3 as smoney ,
 	id4 as salary_label
from
wlfinance.t_zlj_tmp_csv where ds='360_bank_detail_dropdup'
)t1 join (
      select
        tel as userid,
          min(id1) as min_timespan,
      ...
      FROM
        wlfinance.t_zlj_tmp_csv
      WHERE ds = '360_bill_detail_dropdup' and id1>0  group by tel
