...
data=rdd.filter(lambda x:x[-1]<15000).sample(False,0.8,100).map(lambda x:array(x))
...
hiveContext.sql('drop table if EXISTS t_zlj_perfer_user_level_mult_t ')
hiveContext.sql('create table t_zlj_perfer_user_level_mult_t as select * from userlevel')
