...
def get_sousuo(ss):
	p =  False
	if "query=" in ss or "q=" in ss or ("word=" in ss and not "sword" in ss) or "wd=" in ss or "keyword=" in ss or "keyWord=" in ss:
		p = True
	if p :
		#print "\t".join(ss)
		return True
...
# spark-submit  --executor-memory 8G  --driver-memory 10G  --total-executor-cores 80 test2.py \
# /user/wrt/cat_map.txt /user/wrt/test/item_家居家纺.dat /user/wrt/test/pageview_家居家纺.dat \
# /user/zlj/wrt/test/jiaju_keyword_itemtitle5
