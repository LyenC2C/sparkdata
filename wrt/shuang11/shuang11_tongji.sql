...
select sum(sold),sum(sold*saleprice) as sales from t_base_ec_shopitem_b where ds = 20161111

--7
--7.1
select root_cat_name,sum(day_sold) as sold ,sum(day_sold_price) as  sales from
(select item_id,day_sold,day_sold_price,root_cat_name from t_base_ec_item_daysale_dev_new where ds = 20151111)t1
JOIN
...
group by root_cat_name
--7.2
select t2.root_cat_name,sum(sold)as sold,sum(sold*saleprice) as sales from
(select * from t_base_ec_shopitem_b where ds = 20161111)t1
JOIN
wlservice.t_wrt_tmp_shuang11_iteminfo t2
ON
t1.item_id = t2.item_id
...
select shop_id,
(
max(cast(desc_score as float) + cast(service_score as float) + cast(desc_highgap as float))
-
min(cast(desc_score as float) + cast(service_score as float) + cast(desc_highgap as float))
)
