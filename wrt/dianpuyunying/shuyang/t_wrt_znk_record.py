...
def twodays(x,y):   #同一个feedid下进行groupby后的结果
    ...
    if len(item_list) == 2: #有两个评论，一个是昨日，一个是今日
        #判断今日和昨日的位置并分别命名赋值
        if len(item_list[0]) == 5:
...
rdd_last = sc.textFile(s).map(lambda x:f2(x))
...
# spark-submit  --executor-memory 6G  --driver-memory 8G  --total-executor-cores 80  t_wrt_znk_record.py 20160919 20160825
