...
def process(line):
    ...
    return "\001".join([valid_jsontxt(i) for i in
                        [repeat, success, app_key, ts_create, params_str, result_str,
                         interface, match, api_type]])


sc = SparkContext(appName="data_backflow")
'''
data = sc.textFile("/commit/data_backflow/ge_ren_xin_xi.json") \
    .map(lambda a: process(a)) \
    .distinct() \
    .filter(lambda a: a is not None) \
    .saveAsTextFile("/user/lel/temp/ge_ren_xin_xi")
'''
data = sc.textFile("/commit/data_backflow/jinrongyonghuhuaxiang.json") \
    .map(lambda a: process(a)) \
    .distinct() \
    .filter(lambda a: a is not None) \
    .saveAsTextFile("/user/lel/temp/portrait")
