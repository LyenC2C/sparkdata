...
def process(line):
    ...
    phones_dim = filter(lambda a: len(a) == 11 or ('-' in a  and len(a) <=13),re.findall(r"\d{3,4}[-]?\d{7,8}",lawer_reduce))
    ...
    if phones_dim and lawer_phone == '':
        for phone_dim in phones_dim:
            if '-' in phone_dim:
                acode = phone_dim.split('-')[0]
                phone_num = phone_dim.split('-')[1]
                phone_dim = phone_dim.replace('-','')
                result.append((phone_dim,acode,phone_num, lawer_name, practice_company, lawer_office, lvshi_info_add, lawer_telphone,
        ...
        if '-' in lawer_phone:
            acode = lawer_phone.split('-')[0]
            phone_num = lawer_phone.split('-')[1]
            lawer_phone = lawer_phone.replace('-','')
            result.append((lawer_phone, acode,phone_num,lawer_name, practice_company, lawer_office, lvshi_info_add, lawer_telphone,
                       professional_qualifications, lawer_reduce, lawer_info_url, lawer_e_mail, lawer_qq, year_of_practice, img_url))
        else:
            acode = ''
            phone_num = lawer_phone
            result.append((lawer_phone, acode,phone_num,lawer_name, practice_company, lawer_office, lvshi_info_add, lawer_telphone,
                       professional_qualifications, lawer_reduce, lawer_info_url, lawer_e_mail, lawer_qq, year_of_practice, img_url))
    return result
