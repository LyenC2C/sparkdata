...
while getopts "n:t:l:" arg #选项后面的冒号表示该选项需要参数
do
  case $arg in
       n)
         count=$OPTARG
...
if [ $? -eq 0 ] ;then
    ...
    if [ $? -eq 0 ] ;then
    hadoop fs -cat /hive/warehouse/wlservice.db/t_lel_ec_xinyan_$date_time""_cd_igk/*  > $date_time""_igk
    hadoop fs -cat /trans/lel/t_lel_ec_xinyan_$date_time""_cd_igk.teltbname > t_lel_ec_xinyan_$date_time""_cd_igk.teltbname
    awk -F '\001' 'NR==FNR && $3!="None"{a[$2]=$1"\t"$3}NR!=FNR{if(($1 in a))print a[$1]"\t"$2"\t"$3}' t_lel_ec_xinyan_$date_time""_cd_igk.teltbname $date_time""_igk > $date_time""_ingk
    #compare to last time
    awk -F '\001' 'NR==FNR{a[$1]=$1}NR!=FNR{if(!($1 in a))print $0}' ../teeth_tel/""$last_tel_date_time $date_time""_ingk > $date_time""_distincted
    ...
    awk -F '\t' '{print $1}' $date_time""_distincted_$count > phone_num
    cat ../teeth_tel/""$last_tel_date_time > ../teeth_tel/teeth_lel_tel_$date_time
    cat phone_num >> ../teeth_tel/teeth_lel_tel_$date_time
    else
