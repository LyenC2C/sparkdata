...
def backfill_continuously(latest_day, days,iteminfo_date):
    ...
    for i in exec_days:
        ...
        command =" bash /home/wrt/sparkdata/lel/airflow/wrt/ec/ec_itemsold_backfill.sh  {last_day} {last_2_days} {iteminfo_date}".format(last_day=last_day,last_2_days=last_2_days,iteminfo_date=iteminfo_date)
        os.system(command)

def backfill_individually(last_day, last_update_day,iteminfo_date):
    command = "bash /home/wrt/sparkdata/lel/airflow/wrt/ec/ec_itemsold_backfill.sh  {last_day} {last_update_day} {iteminfo_date}".format(last_day=last_day,last_update_day=last_update_day,iteminfo_date=iteminfo_date)
    os.system(command)
