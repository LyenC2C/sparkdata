...
def upload(server_host,usr_name,passwd,local_file_path,remote_file_path,port='21'):
    global UPLOAD
    try:
        return UPLOAD.upload(str(server_host),str(port),str(usr_name),str(passwd),str(remote_file_path),str(local_file_path))
    except Exception, e:
        print str(e)
        return False
...
if __name__ == '__main__':
    '''
    import os
    localpath='e:/taobao.multithread.log'
    print os.stat(localpath).st_size

    path1='~'
    path2='tmmm'
    sfpath='./files/now.test.160611'
    bname=os.path.basename(sfpath)
    remote_path=os.path.join('',bname)
    print bname,remote_path
    '''
    #print os.path.join(path1,path2)

    #249
    print upload('171.212.166.74','ftp249','123456bcd','E://10w.taobao.itemid','/media/disk1/data/10w.taobao.itemid.tm1')
    #lf = MyFTP()
    #lf.ConnectFTP("112.193.146.28","21","zekn","test1234")
    #lf.download("10.18.111.70","21","user","passwd","/localfile","remotefile")
    #lf.upload("112.193.146.28","21","zekn","test1234","160701.test.json.1",'F://160701.test.json')
