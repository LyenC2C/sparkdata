...
lastday_rows=`impala-shell -k -s hive -i cs107 -q "SELECT count(*) FROM $database.$table where ds='$lastday'"`
...
last_2_days_rows=`impala-shell -k -s hive -i cs107 -q "SELECT count(*) FROM $database.$table where ds='$last_2_days'"`
