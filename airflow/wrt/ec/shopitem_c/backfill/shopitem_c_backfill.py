...
def backfill_continuously(latest_day, days):
    ...
    for i in exec_days:
        ...
        command = "bash /home/wrt/sparkdata/airflow/wrt/ec/shopitem_c/backfill/shopitem_c_backfill.sh {last_day} {last_2_days}".format(
            last_day=last_day, last_2_days=last_2_days)
        os.system(command)

def backfill_individually(last_day, last_update_day):
    command = "bash /home/wrt/sparkdata/airflow/wrt/ec/shopitem_c/backfill/shopitem_c_backfill.sh {last_day} {last_update_day}".format(
