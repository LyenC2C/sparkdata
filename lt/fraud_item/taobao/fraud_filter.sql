...
5.正常词汇筛选
"卡拉卡拉|打印纸|充值|热敏纸|收银|logo|海报|走势图|工作服|T恤|收藏册|书籍|书|摇奖机|箱|纸牌|摇号机|购彩金|元|充值|卡包|卡套|卡夹|小票纸|PPT|考勤|单据|设计|名片|网站建设|视频|留学|翻译|现货|全套"
...
and length(regexp_replace(title,'书籍|LOGO|logo|海报|热敏纸|打印纸|小票纸|PPT|收银纸|卡包|卡套|T恤|工作服|卡夹|考勤|现货|全套',''))=length(title)
and keywords<>'玖富|口子'
union all
select * from t_lt_base_tb_item_filter_words_all
...
and length(regexp_replace(title,'单据|设计|LOGO|logo|名片|网站建设|视频|现货|全套',''))=length(title);
...
insert into table wl_analysis.t_lt_crawler_fraud_taobao_item_week partition(ds='20170511')
SELECT t2.* from
(SELECT item_id from wl_service.t_lt_base_sp_recom_item_fraud_all)t1
left join
(select item_id,cat_id,seller_id from wl_base.t_base_ec_item_dev_new ds='20170506')t2
on t1.item_id=t2.item_id
