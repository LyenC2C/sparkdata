...
"金融一体机|质押|典当|乐富|低息|速贷|信用卡|网赚|包装小贷|包装公司|抵押|提现|借呗|黑户|操作|技术|跳码|燃眉之急|信用贷|借钱|借贷|额度|借款|芝麻信用|急用钱|急借钱|典当行|放贷|信用卡还款|拆借|快借|资金困难|资金短缺|放米|快钱|套现|变现|t现|急缺钱|口子|急需资金|抢标|秒标|不需担保|无担保|秒批|秒到|提额|降额|放款|下款|缺钱转|点刷|卡乐付|pos机|刷卡机|手刷|一清|多商户|实时到账|费率|乐刷|易付宝|poss机|ps机|蓝牙POS|蓝牙刷卡机|蓝牙刷卡器|POS刷卡器|刷卡器费率|手机POSS|收款宝|拉卡拉|刷卡机|变现|P2P|蓝牙|pos刷卡器|速刷|汇付|秒到账|手机pos|实时到帐|贷款|车贷|机子|手机刷卡器"

2. 资料造假类(银行流水,企业邮箱账号,社保公积金,学历,公司坏账,工资流水)
"银行对账单|提额认证|闪银认证|闪银提额|不良记录|身份证抠图|代做企业对公流水账|代做普通流水账|银行贷款报表|处理逾期|经营异常|借款逾期|账单技术|代过人脸识别|代过各种人脸识别|闪银|贷款|借款|cpa注册"

"贷款|征信代打|电核流水|销户|代做|银行流水|工资流水|收入证明|社保|公积金|挂靠|代缴|代交|邮箱批发|企业邮箱|邮箱|单位邮箱|公司邮箱|邮箱账号|闪银|Wecash闪银|闪电|51人品|资金周转|学历证书|学位证书|流水回单"

2.1.黑中介类(电话代接,公司贷款包装)
"贷款包装|包过电核|固话代接|代接回访|代接固话|信用卡|银行回访|单位包装|贷款回访|电话回访|回访专用|单位回访|公积金转移|电话代接|回访|呼叫转移|银行小贷|下卡|代接"
...
sku  map<string, string>  COMMENT '商品描述',
ts STRING COMMENT '采集时间戳',
fraud_score_1 BIGINT COMMENT '套现贷款',
fraud_score_11 BIGINT COMMENT '',
fraud_score_2 BIGINT COMMENT '造假中介',
fraud_score_22 BIGINT COMMENT '',
fraud_score_3 BIGINT COMMENT '彩票兼职',
fraud_score_4 BIGINT COMMENT '法律咨询',
fraud_score_d BIGINT COMMENT '正常词数',
...
--1-2
CREATE TABLE  if not exists wl_service.t_lt_base_sp_item_filter_words_all (
item_id STRING  COMMENT  '商品id',
title  STRING   COMMENT '商品title',
cat_id  STRING   COMMENT '商品所属类目id',
cat_name STRING  COMMENT '商品所属类目名称',
...
max(fraud_score_1) as fraud_score_1,max(fraud_score_11) as fraud_score_11,
max(fraud_score_2) as fraud_score_2,max(fraud_score_22) as fraud_score_22,
max(fraud_score_3) as fraud_score_3,max(fraud_score_4) as fraud_score_4,
max(fraud_score_d) as fraud_score_d,collect_set(keywords)[0] as keywords,collect_set(keywords_d)[0] as keywords_d
from t_lt_base_sp_item_filter_words_all
...
union all
select * from t_lt_base_sp_item_filter_words_sub
where (fraud_score_1>1 or fraud_score_11>2) and title like '%玩转信用卡%' and title like '%玩的就是信用卡%'
union all
select * from t_lt_base_sp_item_filter_words_sub
where (fraud_score_1=1 and fraud_score_11=2)
and cat_name not in ('数码收纳整理包','收银纸','电源/适配器')
...
from
(select * from t_lt_base_sp_item_fraud_txdk_1
union all
select * from t_lt_base_sp_item_fraud_zjzj_2
union all
select * from t_lt_base_sp_item_fraud_cpjz_3
union all
select * from t_lt_base_sp_item_fraud_flzx_4)t
group by item_id,title,cat_id,cat_name,root_cat_name,seller_id;
