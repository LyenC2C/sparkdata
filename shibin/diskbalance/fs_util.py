...
def lspath(path):
    ...
    if status!=0:
        raise Exception("unable run  \" ls "+path +" \" error ,please check it")
    return out
...
def get_subdir_host(dir):
    ...
    for line in out.split('\n'):
        if 'BP' in line:
            ...
            return host
    raise Exception("there is'nt hadoop subdir host for the  dir:\" "+dir +" \" ,please check it")
...
def safe_mv(from_host,to_host,from_subdir,block_pair):
    ...
    try:        
        ...
        istrue_block,out=copyfile(from_path+"/"+block,to_path+"/"+block)
        print 'mv| try mv '+from_path+"/"+block_meta+' to:'+to_path+"/"+block_meta
        istrue_block_meta,out=copyfile(from_path+"/"+block_meta,to_path+"/"+block_meta)
